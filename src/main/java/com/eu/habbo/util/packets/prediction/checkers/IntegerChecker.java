...
public class IntegerChecker extends TypeChecker<Integer> {
    ...
    IntegerChecker(SPacket hPacket) {
        super("i", hPacket);
    }

    @Override
    public boolean canRead(int index) {
        return index >= 0 && !(index + 4 > hPacket.getBytesLength());
    }
    ...
    public double score(int index) {
        ...
        int ushortTest = hPacket.readShort(index);
        int ushortTest2 = hPacket.readShort(index + 2);
