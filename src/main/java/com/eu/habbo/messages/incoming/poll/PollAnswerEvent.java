...
public class PollAnswerEvent extends MessageHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(PollAnswerEvent.class);
            ...
            if (poll.lastQuestionId == questionId) {
                if (poll.badgeReward.length() > 0) {
                    if (!this.client.getHabbo().getInventory().getBadgesComponent().hasBadge(poll.badgeReward)) {
                        ...
                        this.client.sendResponse(new WiredRewardResultComposer(WiredRewardResultComposer.REWARD_RECEIVED_BADGE));
                    } else {
                        this.client.sendResponse(new WiredRewardResultComposer(WiredRewardResultComposer.REWARD_ALREADY_RECEIVED));
                    }
                }
            }
        }
