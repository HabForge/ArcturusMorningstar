package com.eu.habbo.messages.incoming.nux;
...
public class NewUserExperienceScriptProceedEvent extends MessageHandler {
    ...
    public static void handle(Habbo habbo) {
        ...
        if (keys.containsKey(step)) {
            habbo.getClient().sendResponse(new InClientLinkComposer("helpBubble/add/" + keys.get(step) + "/" + Emulator.getTexts().getValue("nux.step." + step)));
        } else if (!habbo.getHabboStats().nuxReward) {


        } else {
            habbo.getClient().sendResponse(new InClientLinkComposer("nux/lobbyoffer/show"));
        }
    }
