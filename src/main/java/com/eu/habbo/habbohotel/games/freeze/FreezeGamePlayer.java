...
import com.eu.habbo.messages.outgoing.room.session.GamePlayerValueComposer;
...
public class FreezeGamePlayer extends GamePlayer {
    ...
    public void addLife() {
        if (this.lives < FreezeGame.MAX_LIVES) {
            this.lives++;
            super.getHabbo().getHabboInfo().getCurrentRoom().sendComposer(new GamePlayerValueComposer(this).compose());
        }
    }
