...
public class StaffAlertCommand extends Command {
    ...
    public boolean handle(GameClient gameClient, String[] params) throws Exception {
        if (params.length > 1) {
            ...
            Emulator.getGameServer().getGameClientManager().sendBroadcastResponse(new NewConsoleComposer(new Message(gameClient.getHabbo().getHabboInfo().getId(), -1, message.toString())).compose(), "acc_staff_chat", gameClient);
        } else {
