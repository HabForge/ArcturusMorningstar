...
import com.eu.habbo.messages.outgoing.moderation.ModeratorUserInfoComposer;
...
public class ModToolManager {
    ...
    public void alert(Habbo moderator, Habbo target, String message, SupportUserAlertedReason reason) {
        ...
        if (target != null)
            alertedEvent.target.getClient().sendResponse(new IssueCloseNotificationComposer(alertedEvent.message));
    }
    ...
    public void updateTicketToMods(ModToolIssue issue) {
        ...
        if (!Emulator.getPluginManager().fireEvent(event).isCancelled()) {
            Emulator.getGameEnvironment().getHabboManager().sendPacketToHabbosWithPermission(new IssueInfoComposer(issue).compose(), Permission.ACC_SUPPORTTOOL);
        }
    }
    ...
    public void closeTicketAsUseless(ModToolIssue issue, Habbo sender) {
        ...
        if (sender != null) {
            sender.getClient().sendResponse(new IssueCloseNotificationComposer(IssueCloseNotificationComposer.USELESS));
        }
    ...
    public void closeTicketAsAbusive(ModToolIssue issue, Habbo sender) {
        ...
        issue.updateInDatabase();
        if (sender != null) {
            sender.getClient().sendResponse(new IssueCloseNotificationComposer(IssueCloseNotificationComposer.ABUSIVE));
        }
    ...
    public void closeTicketAsHandled(ModToolIssue issue, Habbo sender) {
        ...
        if (sender != null) {
            sender.getClient().sendResponse(new IssueCloseNotificationComposer(IssueCloseNotificationComposer.HANDLED));
        }
