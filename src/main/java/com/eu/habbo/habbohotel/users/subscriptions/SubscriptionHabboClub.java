...
import com.eu.habbo.messages.outgoing.notifications.ClubGiftNotificationComposer;
import com.eu.habbo.messages.outgoing.room.engine.UserChangeComposer;
import com.eu.habbo.messages.outgoing.users.ScrSendKickbackInfoComposer;
...
public class SubscriptionHabboClub extends Subscription {
    ...
    public void onExpired() {
        ...
        if (habbo != null && ClothingValidationManager.VALIDATE_ON_HC_EXPIRE) {
            ...
            if (habbo.getHabboInfo().getCurrentRoom() != null) {
                habbo.getHabboInfo().getCurrentRoom().sendComposer(new UserChangeComposer(habbo).compose());
            }
        }
     ...
     */
    public static ScrSendKickbackInfoComposer calculatePayday(HabboInfo habbo) {
        ...
        return new ScrSendKickbackInfoComposer(
                currentHcStreak,
            ...
            try (ResultSet set = statement.executeQuery()) {
                while (set.next()) {
                    try {
                        ...
                        HabboStats stats = habboInfo.getHabboStats();
                        ScrSendKickbackInfoComposer calculated = calculatePayday(habboInfo);
