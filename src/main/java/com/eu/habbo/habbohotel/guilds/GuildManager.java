                ...
                try (ResultSet set = statement.executeQuery()) {
                    if (set.next()) {
                        if (set.getInt(1) >= 100) {
                            ...
                            if (userId == 0)
                                client.sendResponse(new HabboGroupJoinFailedComposer(HabboGroupJoinFailedComposer.GROUP_LIMIT_EXCEED));
                            else
                                client.sendResponse(new HabboGroupJoinFailedComposer(HabboGroupJoinFailedComposer.MEMBER_FAIL_JOIN_LIMIT_EXCEED_NON_HC));
                    ...
                    try (ResultSet set = statement.executeQuery()) {
                        if (set.next()) {
                            if (set.getInt(1) >= 50000) {
                                client.sendResponse(new HabboGroupJoinFailedComposer(HabboGroupJoinFailedComposer.GROUP_FULL));
                                error = true;
                            }
                        }
                    }
                    ...
                    if (guild.getState() == GuildState.EXCLUSIVE) {
                        try (PreparedStatement statement = connection.prepareStatement("SELECT COUNT(id) as total FROM guilds_members WHERE guild_id = ? AND level_id = 3")) {
                            ...
                            try (ResultSet set = statement.executeQuery()) {
                                if (set.next()) {
                                    if (set.getInt(1) >= 100) {
                                        client.sendResponse(new HabboGroupJoinFailedComposer(HabboGroupJoinFailedComposer.GROUP_NOT_ACCEPT_REQUESTS));
                                        error = true;
                                    }
                                }
                            }
