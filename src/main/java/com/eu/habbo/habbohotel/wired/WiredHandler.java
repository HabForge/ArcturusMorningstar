...
import com.eu.habbo.messages.outgoing.userdefinedroomevents.WiredRewardResultComposer;
...
public class WiredHandler {
    ...
    private static void giveReward(Habbo habbo, WiredEffectGiveReward wiredBox, WiredGiveRewardItem reward) {
        ...
        if (reward.badge) {
            ...
            habbo.getClient().sendResponse(new WiredRewardResultComposer(WiredRewardResultComposer.REWARD_RECEIVED_BADGE));
        } else {
                ...
                if (rewardReceived.type.equalsIgnoreCase("credits")) {
                    ...
                    if (baseItem != null) {
                        ...
                        if (item != null) {
                            ...
                            habbo.getClient().sendResponse(new WiredRewardResultComposer(WiredRewardResultComposer.REWARD_RECEIVED_ITEM));
                        }
                    }
    ...
    public static boolean getReward(Habbo habbo, WiredEffectGiveReward wiredBox) {
        if (wiredBox.limit > 0) {
            if (wiredBox.limit - wiredBox.given == 0) {
                habbo.getClient().sendResponse(new WiredRewardResultComposer(WiredRewardResultComposer.LIMITED_NO_MORE_AVAILABLE));
                return false;
            }
        }
                ...
                if (set.first()) {
                    if (set.getInt("row_count") >= 1) {
                        if (wiredBox.rewardTime == WiredEffectGiveReward.LIMIT_ONCE) {
                            habbo.getClient().sendResponse(new WiredRewardResultComposer(WiredRewardResultComposer.REWARD_ALREADY_RECEIVED));
                            return false;
                        }
                    }
                    ...
                    if (set.next()) {
                        if (wiredBox.rewardTime == WiredEffectGiveReward.LIMIT_N_MINUTES) {
                            if (Emulator.getIntUnixTimestamp() - set.getInt("timestamp") <= 60) {
                                habbo.getClient().sendResponse(new WiredRewardResultComposer(WiredRewardResultComposer.REWARD_ALREADY_RECEIVED_THIS_MINUTE));
                                return false;
                            }
                        }
                        ...
                        if (wiredBox.uniqueRewards) {
                            if (set.getInt("row_count") == wiredBox.rewardItems.size()) {
                                habbo.getClient().sendResponse(new WiredRewardResultComposer(WiredRewardResultComposer.REWARD_ALL_COLLECTED));
                                return false;
                            }
                        }
                        ...
                        if (wiredBox.rewardTime == WiredEffectGiveReward.LIMIT_N_HOURS) {
                            if (!(Emulator.getIntUnixTimestamp() - set.getInt("timestamp") >= (3600 * wiredBox.limitationInterval))) {
                                habbo.getClient().sendResponse(new WiredRewardResultComposer(WiredRewardResultComposer.REWARD_ALREADY_RECEIVED_THIS_HOUR));
                                return false;
                            }
                        }
                        ...
                        if (wiredBox.rewardTime == WiredEffectGiveReward.LIMIT_N_DAY) {
                            if (!(Emulator.getIntUnixTimestamp() - set.getInt("timestamp") >= (86400 * wiredBox.limitationInterval))) {
                                habbo.getClient().sendResponse(new WiredRewardResultComposer(WiredRewardResultComposer.REWARD_ALREADY_RECEIVED_THIS_TODAY));
                                return false;
                            }
                        }
                    }
