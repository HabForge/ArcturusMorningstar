...
import com.eu.habbo.messages.outgoing.inventory.pets.ConfirmBreedingRequestComposer;
import com.eu.habbo.messages.outgoing.inventory.pets.NestBreedingSuccessComposer;
...
public class InteractionPetBreedingNest extends HabboItem {
    ...
    public void stopBreeding(Habbo habbo) {
        ...
        if (this.petOne != null) {
            habbo.getClient().sendResponse(new OpenPetPackageResultComposer(this.getId(), OpenPetPackageResultComposer.CLOSE_WIDGET, ""));
        }
        if (this.petTwo.getUserId() != habbo.getHabboInfo().getId()) {
            Habbo owner = this.petTwo.getRoom().getHabbo(this.petTwo.getUserId());
            if (owner != null) {
                owner.getClient().sendResponse(new OpenPetPackageResultComposer(this.getId(), OpenPetPackageResultComposer.CLOSE_WIDGET, ""));
            }
        }
    ...
    public void breed(Habbo habbo, String name, int petOneId, int petTwoId) {
        ...
        Emulator.getThreading().run(() -> {
            ...
            //habbo.getClient().sendResponse(new OpenPetPackageResultComposer(box.getId(), OpenPetPackageResultComposer.CLOSE_WIDGET, ""));
