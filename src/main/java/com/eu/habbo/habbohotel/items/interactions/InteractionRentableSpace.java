...
import com.eu.habbo.messages.outgoing.room.furniture.RentableSpaceStatusComposer;
...
public class InteractionRentableSpace extends HabboItem {
    ...
    public int getRentErrorCode(Habbo habbo) {
        if (this.isRented() && this.renterId != habbo.getHabboInfo().getId()) {
            return RentableSpaceStatusComposer.SPACE_ALREADY_RENTED;
        }

        if (habbo.getHabboStats().isRentingSpace() && habbo.getHabboStats().getRentedItemId() != this.getId()) {
            return RentableSpaceStatusComposer.CAN_RENT_ONLY_ONE_SPACE;
        }

        if (habbo.getHabboStats().getClubExpireTimestamp() < Emulator.getIntUnixTimestamp()) {
            return RentableSpaceStatusComposer.CANT_RENT_NO_HABBO_CLUB;
        }

        if (this.rentCost() > habbo.getHabboInfo().getCredits()) {
            return RentableSpaceStatusComposer.NOT_ENOUGH_CREDITS;
        }
    ...
    public void sendRentWidget(Habbo habbo) {
        habbo.getClient().sendResponse(new RentableSpaceStatusComposer(habbo, this, this.getRentErrorCode(habbo)));
    }
}
